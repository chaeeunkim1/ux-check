...
async function generateDraft(input,{fetcher=fetch,apiKey=process.env.ANTHROPIC_API_KEY,model=process.env.ANTHROPIC_MODEL||'claude-sonnet-4-6',timeoutMs=110000}={}) {
  ...
  const content=[{type:'text',text:'첫 번째 이미지는 원본(before) 화면입니다.'},input.before];
  if(input.after)content.push({type:'text',text:'두 번째 이미지는 개선 후(after) 화면입니다.'},input.after);
  content.push({type:'text',text:JSON.stringify({mode:input.mode,purpose:input.purpose,instructions:input.mode==='compare'?'각 항목의 status를 resolved, remaining, new, uncertain 중 하나로 분류합니다. 전후 차이를 comparison에 설명하세요. 개선 후 남은 문제와 새 문제는 after 화면 위치를, 해결된 문제는 before 화면 위치를 사용합니다.':'status는 observed, image는 before, comparison은 빈 문자열로 지정하세요.'})});
  let response;
  try{response=await fetcher('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'x-api-key':apiKey,'anthropic-version':'2023-06-01','Content-Type':'application/json'},body:JSON.stringify({model,max_tokens:6000,system:SYSTEM,messages:[{role:'user',content}],tools:[{name:'submit_review',description:'화면에서 관찰한 UI/UX 검수 결과를 제출합니다. 한국어 근거와 개선안을 포함하고 모든 필드는 스키마를 따릅니다. 이 도구는 외부 작업을 실행하지 않고 구조화된 보고서를 반환합니다.',input_schema:reviewSchema}],tool_choice:{type:'tool',name:'submit_review'}}),signal:AbortSignal.timeout(timeoutMs)});}catch(error){throw new ReviewError(error.name==='TimeoutError'?'timeout':'upstream_unavailable',error.name==='TimeoutError'?504:502,error.name==='TimeoutError'?'분석 시간이 길어졌습니다. 화면을 단순하게 잘라 다시 시도해 주세요.':'AI 서비스에 연결하지 못했습니다. 잠시 후 다시 시도해 주세요.');}
  ...
  let report;try{report=validateReport(tool?.input,input.mode);}catch{throw new ReviewError('invalid_response',502,'분석 결과의 형식이 올바르지 않습니다. 다시 분석해 주세요.');}
