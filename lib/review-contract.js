...
export function reportMarkdown(report) {
  const lines=['# UX Check · UI/UX 검수 보고서','',`- 검수 방식: ${report.mode==='compare'?'개선 전후 비교':'화면 검수'}`,`- 생성 시각: ${report.createdAt}`,`- 분석 모델: ${report.model}`,`- 결과 출처: ${report.source==='saved-example'?'저장된 합성 예시 · 현재 실시간 분석 아님':'실제 API 분석'}`,`- 업무 목적: ${report.purpose}`,'','## 검수 요약','',report.summary,'','> AI 제안이며 최종 판단은 담당자가 합니다. 위치 표시는 근사치입니다. 스크린샷만으로 실제 동작이나 접근성 준수를 확정하지 않습니다.',''];
