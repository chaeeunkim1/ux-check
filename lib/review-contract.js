...
export function validateReport(value,mode) {
  ...
  const issues=value.issues.map((item,index)=>{
    ...
    if(mode==='site'&&(item.status!=='observed'||!['before','after'].includes(item.image)))throw new Error('site_mode_status');
