...
export function siteAnalysisFacts(capture){
 ...
 return JSON.stringify({url:capture.finalUrl,scope:capture.scope,desktop:overview('desktop'),mobile:overview('mobile'),screens:capture.screens?.map(s=>({key:s.key,label:s.label,scrollY:s.scrollY,viewport:s.viewport,observations:s.observations?.slice(0,12)}))}).slice(0,18000);
}
