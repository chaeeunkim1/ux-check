...
export async function validateReviewInput(body) {
  ...
  if(body.mode==='site'&&body.siteContext){
    ...
    siteContext={url:url.href,title:String(body.siteContext.title||'').slice(0,200),scope:'공개 페이지 1개 · 데스크톱 1280×900 / 모바일 390×844 첫 화면 · 클릭 및 입력 미실행'};
  }
  return {mode:body.mode,purpose,before,after,siteFacts,siteContext};
}
