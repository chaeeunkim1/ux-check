...
export async function validateReviewInput(body) {
  if(!body||!['review','compare'].includes(body.mode))throw new ReviewError('invalid_mode',400,'검수 방식을 선택해 주세요.');
  ...
  const after=body.mode==='compare'?await validateImage(body.after):null;
  return {mode:body.mode,purpose,before,after};
}
