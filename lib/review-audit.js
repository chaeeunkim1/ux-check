...
export async function auditReview(input,draft,{fetcher=fetch,apiKey=process.env.ANTHROPIC_API_KEY,model=process.env.ANTHROPIC_MODEL||'claude-sonnet-4-6'}={}){
 ...
 let response;
 try{response=await fetcher('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'x-api-key':apiKey,'anthropic-version':'2023-06-01','Content-Type':'application/json'},body:JSON.stringify({model,max_tokens:3000,system:SYSTEM,messages:[{role:'user',content}],tools:[{name:'verify_review',description:'초안의 모든 항목을 이미지와 대조한 뒤 근거 여부, 정확한 제목, 적정 우선순위와 이유를 반환합니다. 삭제 대신 unsupported 판정을 반환하며 number는 원본 번호를 그대로 사용합니다. 실제 기능을 실행한 것으로 표현하지 않습니다.',input_schema:auditSchema}],tool_choice:{type:'tool',name:'verify_review'}}),signal:AbortSignal.timeout(70000)});}catch(error){throw new ReviewError('verification_unavailable',error.name==='TimeoutError'?504:502,'검수 근거를 다시 확인하는 중 연결이 끊겼습니다. 다시 시도해 주세요.');}
