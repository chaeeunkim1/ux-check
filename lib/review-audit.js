...
export async function auditReview(input,draft,{fetcher=fetch,apiKey=process.env.ANTHROPIC_API_KEY,model=process.env.ANTHROPIC_MODEL||'claude-sonnet-4-6'}={}){
 ...
 content.push({type:'text',text:JSON.stringify({mode:input.mode,purpose:input.purpose,...(input.mode==='site'?{siteFacts:input.siteFacts,instructions:'동일 사이트의 데스크톱/모바일 관찰입니다. 개선 전후 판정을 하지 마세요. 첫 화면 밖의 동작은 확정하지 마세요.'}:{}),draft:{summary:draft.summary,issues:draft.issues}})});
 let response;
