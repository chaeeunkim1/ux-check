import {createCaptureRequestBudget} from './capture-budget.js';
...
export async function captureSite(value){
 const url=normalizeSiteUrl(value),requestBudget=createCaptureRequestBudget();let proxy,browser;const resourceSizes=[];const started=Date.now();let blocked=0,requestCount=0,limitBlocked=0,mainNavigationBlocked=0,lastProtocol='',lastNavigationFailure='',stage='public_dns';
 try{
  await resolvePublicHost(url.hostname);proxy=await createPublicProxy();stage='browser_start';
  ...
  const page=await browser.newPage();const session=await page.createCDPSession();await session.send('Emulation.setUserAgentOverride',{userAgent:await browser.userAgent(),acceptLanguage:'ko-KR,ko;q=0.9,en;q=0.8'});await page.setBypassServiceWorker(true);await page.setRequestInterception(true);
  page.on('request',request=>{
   try{
    const resource=request.resourceType();
    requestCount++;
    if(!requestBudget.take()){limitBlocked++;if(request.isNavigationRequest()&&request.frame()===page.mainFrame())mainNavigationBlocked++;blocked++;return void request.abort().catch(()=>{});}
    if(!['GET','HEAD'].includes(request.method())||['websocket','eventsource','media','manifest'].includes(resource)||(request.isNavigationRequest()&&request.frame()!==page.mainFrame())){blocked++;return void request.abort().catch(()=>{});}
  ...
  page.on('response',response=>{const length=Number(response.headers()['content-length']||0);if(length>0){resourceSizes.push({type:response.request().resourceType(),mime:response.headers()['content-type']||'',bytes:length});resourceSizes.sort((a,b)=>b.bytes-a.bytes);resourceSizes.length=Math.min(resourceSizes.length,6);}});
  page.on('requestfailed',request=>{if(request.isNavigationRequest()&&request.frame()===page.mainFrame())lastNavigationFailure=String(request.failure()?.errorText||'').match(/ERR_[A-Z_]+/)?.[0]||'navigation_failed';});
  ...
  stage='navigation_url';normalizeSiteUrl(page.url());
  ...
  if(proxy.stats().byteLimitExceeded)throw new ReviewError('page_resource_limit',422,'페이지의 다운로드 용량이 한도를 넘었습니다. 더 단순한 하위 페이지로 검사해 주세요.');
  stage='mobile_capture';requestBudget.startViewport();proxy.startViewport();
  ...
  stage='final_url';lastProtocol=new URL(page.url()).protocol;if(proxy.stats().byteLimitExceeded)throw new ReviewError('page_resource_limit',422,'페이지의 다운로드 용량이 한도를 넘었습니다. 더 단순한 하위 페이지로 검사해 주세요.');if(mainNavigationBlocked)throw new ReviewError('page_resource_limit',422,'페이지의 파일 요청이 많아 화면을 모두 불러오지 못했습니다. 더 단순한 하위 페이지로 검사해 주세요.');if(!['http:','https:'].includes(lastProtocol))throw new ReviewError('page_loading_failed',422,'사이트 화면을 불러오는 중 연결이 끊겼습니다. 잠시 후 다시 시도해 주세요.');const finalUrl=normalizeSiteUrl(page.url()).href;
  console.info('site_capture_complete',JSON.stringify({durationMs:Date.now()-started,requests:requestCount,blocked,screens:screens.length,proxy:proxy.stats()}));
 ...
 }catch(error){if(error instanceof ReviewError){console.warn('site_capture_rejected',JSON.stringify({stage,code:error.code,requests:requestCount,blocked,limitBlocked,mainNavigationBlocked,lastProtocol,lastNavigationFailure,proxy:proxy?.stats(),largestResources:resourceSizes}));throw error;}console.warn('site_capture_failed',JSON.stringify({stage,name:error?.name,networkCode:String(error?.message||'').match(/ERR_[A-Z_]+/)?.[0],requests:requestCount,blocked}));throw new ReviewError('capture_failed',422,'페이지를 열지 못했습니다. 사이트의 접속 제한·인증 또는 로딩 지연이 있을 수 있습니다. 다른 공개 주소로 다시 시도해 주세요.');}
