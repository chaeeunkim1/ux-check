...
export async function captureSite(value){
 ...
 try{
  ...
  browser=await puppeteer.launch({executablePath,headless:true,acceptInsecureCerts:false,args:[...(isLocal?[]:chromium.args),`--proxy-server=${proxy.url}`,'--proxy-bypass-list=<-loopback>','--disable-quic','--disable-background-networking','--disable-component-update','--disable-extensions','--force-webrtc-ip-handling-policy=disable_non_proxied_udp','--disable-features=WebRtcAllowInputVolumeAdjustment'],timeout:20000});
