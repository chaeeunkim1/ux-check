...
import { createCaptureByteBudget } from './capture-budget.js';
export async function createPublicProxy(){
 const sockets=new Set(),byteBudget=createCaptureByteBudget();let connections=0,closed=false;
 ...
 server.on('connect',async(req,client,head)=>{
  try{
   if(closed||byteBudget.stats().byteLimitExceeded||++connections>140)throw Error('limit');
 ...
 return {url:`http://127.0.0.1:${server.address().port}`,startViewport:()=>byteBudget.startViewport(),close:()=>{closed=true;for(const socket of sockets)socket.destroy();server.close();},stats:()=>({...byteBudget.stats(),connections})};
}
