...
1. `전후 비교` 탭에서 `개선 전 캡처`와 `개선 후 캡처`에 실제 PNG/JPG/WebP 파일을 각각 올립니다. 업로드 순서는 자유롭습니다.
2. 같은 업무 목적, 화면 크기와 캡처 구간을 맞추고 업무 목적을 입력합니다.
3. 공유 가능한 화면의 외부 AI 전송에 동의한 뒤 `개선 전후 비교하기`를 누릅니다.
4. 개선 확인·남은 문제·새 문제·추가 확인 항목과 전후 근거를 확인하고 보고서를 저장합니다.

합성 예제는 접힌 체험 메뉴에서 직접 선택한 경우에만 채워집니다. 실제 파일로 교체하면 반대편 합성 예제도 비워 잘못된 조합을 방지합니다. 두 실제 파일은 연속 업로드하거나 한 장을 바꿔도 서로 유지됩니다. 화면에서 보이는 차이를 분석하며 클릭 동작의 개선까지 검증하지는 않습니다.
