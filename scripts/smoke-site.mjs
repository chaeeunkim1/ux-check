...
async function post(path,body){const response=await fetch(base+path,{method:'POST',headers:{'Content-Type':'application/json',Origin:new URL(base).origin},body:JSON.stringify(body),signal:AbortSignal.timeout(250000)});const data=await response.json().catch(()=>({error:'invalid_json'}));return {status:response.status,data};}
...
if(captured.status!==200){await writeFile(directory+'/result.json',JSON.stringify(captured,null,2));console.log(JSON.stringify({stage:'capture',status:captured.status,error:captured.data.error,directory}));process.exitCode=1;}else{
 ...
 const {desktop,mobile,...metadata}=capture;await writeFile(directory+'/capture.json',JSON.stringify(metadata,null,2));
 ...
 if(option!=='--capture-only'){
  const analyzed=await post('/api/review',{mode:'site',purpose:'방문자가 사이트의 주요 정보를 이해하고 필요한 메뉴를 찾아 다음 행동을 쉽게 수행한다.',consent:true,before:desktop.data,after:mobile.data,siteContext:{url:capture.finalUrl,title:capture.title},siteFacts:JSON.stringify({url:capture.finalUrl,...capture.facts,scope:capture.scope})});
